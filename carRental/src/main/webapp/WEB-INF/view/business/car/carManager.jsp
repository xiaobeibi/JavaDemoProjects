...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'form', 'table', 'upload'], function () {
        ...
        tableIns = table.render({
            ...
            , url: '${alfred}/car/loadAllCar.action' //数据接口
            ...
            , cols: [[   //列表数据
                ...
                , {
                    field: 'carimg', title: '缩略图', align: 'center', width: '80', templet: function (d) {
                        return "<img width=40 height=40 src=${alfred}/file/downloadShowFile.action?path=" + d.carimg + "/>";
                    }
                }
...
//            alert(params);
            tableIns.reload({
                url: "${alfred}/car/loadAllCar.action?" + params,
                page: {curr: 1}
            })
        });
        ...
        table.on('tool(carTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.carnumber + '】这个车辆么？', function (index) {
                    //向服务端发送删除指令
                    $.post("${alfred}/car/deleteCar.action", {carnumber: data.carnumber}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
        ...
        function openAddCar() {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    $("#dataFrm")[0].reset();
                    //设置默认图片
                    $("#showCarImg").attr("src", "${alfred}/file/downloadShowFile.action?path=images/defaultcarimage.jpg");
                    $("#carimg").val("images/defaultcarimage.jpg");
                    url = "${alfred}/car/addCar.action";
        ...
        function openUpdateCar(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    $("#showCarImg").attr("src", "${alfred}/file/downloadShowFile.action?path=" + data.carimg);
                    url = "${alfred}/car/updateCar.action";
        ...
        function deleteBatch() {
            ...
            layer.confirm('真的要删除这些车辆么？', function (index) {
                //向服务端发送删除指令
                $.post("${alfred}/car/deleteBatchCar.action", params, function (res) {
                    layer.msg(res.msg);
                    //刷新数据表格
                    tableIns.reload();
                })
        ...
        upload.render({
            ...
            field: "mf",
                ...
                $('#showCarImg').attr('src', "${alfred}/file/downloadShowFile.action?path=" + res.data.src);
        ...
        function showCarImage(data) {
            mainIndex = layer.open({
                ...
                area: ['750px', '500px'],
                success: function (index) {
                    $("#view_carimg").attr("src","${alfred}/file/downloadShowFile.action?path="+data.carimg);
                }
            });
        }
